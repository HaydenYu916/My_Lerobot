...
def sample_beta(alpha, beta, bsize, device):  # see openpi `sample_beta` (exact copy)
    # Beta sampling uses _sample_dirichlet which isn't implemented for MPS, so sample on CPU for MPS only.
    # On CUDA we sample directly on GPU to avoid CPU->GPU sync/copies (and to keep cudagraphs enabled).
    device = torch.device(device)
    dist_device = torch.device("cpu") if device.type == "mps" else device

    alpha_t = torch.as_tensor(alpha, dtype=torch.float32, device=dist_device)
    beta_t = torch.as_tensor(beta, dtype=torch.float32, device=dist_device)
    dist = torch.distributions.Beta(alpha_t, beta_t)
    sample = dist.sample((bsize,))
    return sample.to(device) if dist_device.type == "cpu" else sample
